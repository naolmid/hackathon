...
export default function LabManagerDashboard() {
  ...
  const handleChangePlaceSubmit = async (data: any) => {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || errorData.details || "Failed to submit movement");
      }
      
      const result = await response.json();
      alert(result.message || "Item moved successfully!");
    } catch (error: any) {
  ...
  const handleSendReportSubmit = async (data: {
    ...
    const response = await fetch("/api/staff/alert", {
      ...
      body: JSON.stringify({
        ...data,
        alertType: data.alertType || "FACILITY_ISSUE",
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-white">Lab Manager Dashboard</h1>
